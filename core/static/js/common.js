...
// Transparently adds X-CSRF-Token header to all POST/PUT/DELETE requests.
// Fetches token lazily on first mutating request, caches it, auto-refreshes on 403.
(function() {
    ...
    async function fetchCsrfToken() {
        try {
            const res = await _originalFetch('/api/csrf-token', { method: 'GET', credentials: 'same-origin' });
            if (res.ok) {
    ...
    async function ensureCsrfToken() {
        ...
        if (!_csrfPromise) {
            _csrfPromise = fetchCsrfToken().finally(() => { _csrfPromise = null; });
        }
        return _csrfPromise;
    }
            ...
            if (response.status === 403) {
                try {
                    ...
                    if (body.error && body.error.toLowerCase().includes('csrf')) {
                        ...
                        if (newToken) {
                            if (init.headers instanceof Headers) {
                                init.headers.set('X-CSRF-Token', newToken);
                            } else {
                                init.headers = { ...init.headers, 'X-CSRF-Token': newToken };
                            }
                            return _originalFetch(resource, init);
                        }
                    }
                } catch (_) { /* not JSON or not CSRF error, return original response */ }
            }
